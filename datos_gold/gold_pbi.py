...
if silver_count == 0:
    raise ValueError("❌ Silver PBI está vacío. No se puede sincronizar.")

print(f"🔎 Filas en Silver PBI: {silver_count}")
...
update_sql = f"""
    ...
    FROM {SILVER_SCHEMA}.{SILVER_TABLE} s
    WHERE g.fecha = s.fecha
    AND g.pbi IS DISTINCT FROM s.pbi;
"""
...
print("\n🚀 Sincronización incremental Silver → Gold PBI completada correctamente.")
